%Read me--------------
% Script for automated processing ASCII files generated by topspin command convbin2asc. This script has initially been made to analyze one deuterium peak from D2O. The first process spectra in topspin using commands ‘efp’, ‘apk’ and ‘absn’. (It is recommended to write user script by using command 'edau' and use ‘qumulti’ afterwards to run this user script for all spectra at once). Afterwards, use command ‘qumulti convbin2asc [numbers of expno]’ to export all spectra into asci files. 
% How to use this script:
% Copy this .m file into the parent folder containing your D2O experiments. Make sure all of the spectra are processed and exported into asci files. Open this .m file in Matlab and click the big green RUN button. 
% Outputs:
